...
func (s *SQLServer) preStart(
	...
	// 1. [租户连接] 如果是租户实例，首先启动租户连接器。
	// 该方法会阻塞，直到建立与集群的连接并确定集群 ID。
	if s.tenantConnect != nil {
		if err := s.tenantConnect.Start(ctx); err != nil {
			return err
		}
		// 将租户名称传播到日志上下文中，以便在日志输出中显示。
	...
	// 2. [版本自举] 从存储中初始化集群版本设置。
	// 这是确保 SQL 层与集群其他部分版本一致的关键步骤。
	...
	if s.execCfg.Codec.ForSystemTenant() {
		...
		// 对于普通租户，由于 service-mode 切换是异步的，可能需要重试以等待 Rangefeed 更新。
	...
	// 3. [配置同步] 启动设置观察器 (Settings Watcher)。
	// 在 Watcher 启动前，系统设置的值是无意义的，大部分子系统都依赖这些设置。
	...
	// 4. [SQL 存活 Session] 启动 SQL Liveness 子系统。
	// 这对于管理租约和 SQL 会话至关重要。
	...
	// 5. [SQL 实例 ID 分配] 通过 SQL 引擎获取或创建实例行。
	// 实例 ID (InstanceID) 是租户内每个 SQL 服务器的唯一标识。
...
// startCheckService 验证租户最初是否具有正确的服务模式（Service Mode），
// 然后启动一个异步检查器，在服务模式发生变化时停止服务器。
func (s *SQLServer) startCheckService(ctx context.Context, stopper *stop.Stopper) error {
	...
	// check 函数从连接器获取最新的租户信息，并验证服务模式是否匹配。
	...
	// 1. [同步检查] 进行初始同步检查。
	// 如果初始服务模式不正确，则直接阻止 SQL 服务启动。
	if _, err := check(); err != nil {
		return err
	}

	// 2. [异步监控] 启动后台任务持续监控服务模式变化。
	return stopper.RunAsyncTask(ctx, "check-tenant-service", func(ctx context.Context) {
		for {
			...
			case <-ctx.Done():
				return
			case <-updateCh: // 当租户元数据发生变化时，updateCh 会被关闭/通知。
...
// checkServerModeMatchesEntry 检查预期的服务模式是否与租户记录中的实际模式一致。
func checkServerModeMatchesEntry(
	...
	if actualMode := entry.ServiceMode; expectedMode != actualMode {
		// 只有在数据状态（DataState）仍然为 Ready 时，才允许优雅停机（Graceful Drain）。
...
// startServeSQL 开始通过 TCP 接受传入的 SQL 连接。
// 如果配置了 Unix 套接字，它也会开始在 Unix 套接字上监听。
func startServeSQL(
	ctx context.Context,
	stopper *stop.Stopper,
	...
	_ = stopper.RunAsyncTaskEx(ctx,
		...
		func(ctx context.Context) {
			err := connManager.ServeWith(ctx, pgL, func(ctx context.Context, conn net.Conn) {
				...
				// [预处理] 处理 SSL 协商、取消请求（Cancel Request）等。
				...
				// [正式处理] 将连接交给 pgServer.ServeConn 进行协议处理和认证。
